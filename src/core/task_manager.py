...
def process_autonomous_task(user_input, system_info, verbose=False):
    """Autonomously process a multi-step task on the real machine with feedback and dynamic retries."""
    task = user_input
    steps = []  # List of dicts with command and result
    error_retries = 0
    last_error = None
    max_retries = config["aish"].get("error_retries", 3)
    attempts = []
    ...
    while True:
        ...
        ai_response = clean_response(query_ai(full_prompt))
        ...
        if ai_response.startswith("Error:"):
            ...
            if error:
                ...
                last_error = error
                error_retries += 1
                attempts.append({'command': command, 'error': error})
                ...
                # Re-prompt AI with error context for correction
                continue
            else:
            ...
            # Unexpected response, treat as error and retry if possible
