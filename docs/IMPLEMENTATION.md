...
The model is responsible for generating commands and optionally summarizing their output. The `aish` binary is responsible for everything around the model: runtime management, safety checks, context, configuration, confirmation, execution, output presentation, history, and future shell integration. Generated commands stay internal during normal use.
...
Execution output has two presentation modes:

```text
raw      - print stdout and stderr unchanged
natural  - summarize stdout and stderr while preserving exact paths,
           file names, identifiers, numbers, URLs, and errors
```
